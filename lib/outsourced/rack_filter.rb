  ...
  class RackFilter
    ...
    def call(env)
      ...
      if token_string = oauth2_token(request)
        if token = Outsourced::Oauth::Oauth2Token.first(:conditions => ['invalidated_at IS NULL AND authorized_at IS NOT NULL and token = ?', token_string])
        ...
        client_application = Outsourced::Oauth::ClientApplication.find_by_key(request_proxy.consumer_key)
        ...
        if env["oauth.token_candidate"]
          ...
          if env["oauth.token"].is_a?(::Outsourced::Oauth::RequestToken)
            strategies << :oauth10_request_token
          elsif env["oauth.token"].is_a?(::Outsourced::Oauth::AccessToken)
            strategies << :token
    ...
    def oauth1_verify(request, options = {}, &block)
      begin
        ...
        return false unless Outsourced::Oauth::OauthNonce.remember(signature.request.nonce, signature.request.timestamp)
